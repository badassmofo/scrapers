...
def decompose hex
  [ hex[0].to_i(16), hex[1].to_i(16), hex[2].to_i(16) ]
end

def normalize rgb
  root = rgb[0] * rgb[0] + rgb[1] * rgb[1] + rgb[2] * rgb[2]
  return [ 0.0, 0.0, 0.0 ] if root < 0.0001
  n = 1.0 / Math.sqrt(root)
  [ rgb[0] * n, rgb[1] * n, rgb[2] * n ]
end

def color_dist rgb, hex
  hex = decompose [ hex[0..1], hex[2..3], hex[4..5] ]
  res = [ rgb[0] - hex[0], rgb[1] - hex[1], rgb[2] - hex[2] ]
  res[0] * res[0] + res[1] * res[1] + res[2] * res[2]
end

def best_match rgb
  best, best_match = nil, 0
  $colors.keys.each do |k|
    dist = color_dist rgb, k
    if best.nil? or dist < best_match
      best       = k
      best_match = dist
    end
  end
  ...
  if $*.empty?
    ...
    $*.each do |x|
      ...
      when /^#?([a-f0-9]{2})([a-f0-9]{2})([a-f0-9]{2})$/i
        puts "#{$~} => #{$colors[best_match decompose $~.to_a[1..-1]]}"
      when /^(\d+)$/
        y = $1.to_i
        abort "Invalid number: #{x}" unless y > 15 and y < 256

        match = false
        $colors.each do |k,v|
          match = k if v == y
          break unless not match
        end
        puts "#{y} => ##{match.upcase}"
      else
        puts "Invalid argument: #{x}"
      end
    end
  end
end
